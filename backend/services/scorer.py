...
def score_application(handle: str, platform: str, video_links: list[str], niche: str = "general") -> dict:
...
- 账号: {handle}
- 目标品类: {niche}
- 过往视频链接: {', '.join(video_links) if video_links else '未提供'}
"""
    try:
        resp = httpx.post(
...
def _mock_score(handle: str) -> dict:
    """无 API Key 时的模拟评分"""
    import hashlib
